...
git config user.name "samarthsoni17"
git config user.email "s.soni87@yahoo.com"
git config --list
# change commit's git username from user@server to what i need it to be (my github)
git commit --amend --author="Samarth Soni <s.soni87@yahoo.com" --no-edit
...
python -m pip install   pandas scikit-learn matplotlib tqdm ipywidgets pyarrow plotly seaborn   scikit-plot pydotplus pydot statsmodels kmodes kaggle   kaleido
pip install "vllm>=0.5" transformers accelerate
pip install "vllm>=0.5" transformers accelerate
pip install -U faiss-cpu sentence-transformers uvicorn[standard] httpx fastapi trafilatura unstructured pydantic
...
#    /home/svu/$USER/.cache/huggingface/token
